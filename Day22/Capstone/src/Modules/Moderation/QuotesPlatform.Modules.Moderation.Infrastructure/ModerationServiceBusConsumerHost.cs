...
/// <summary>
/// Competing-consumer worker for ServiceBusTopology.Subscriptions.ModerationReviewRequests.
///
...
/// </summary>
public sealed class ModerationServiceBusConsumerHost(
    IServiceScopeFactory scopeFactory,
    ServiceBusClient serviceBusClient,
    ILogger<ModerationServiceBusConsumerHost> logger) : BackgroundService
{
    private const string SubscriptionName = ServiceBusTopology.Subscriptions.ModerationReviewRequests;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        logger.LogInformation("Moderation Service Bus processor started on {Subscription}", SubscriptionName);
        ...
        try
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // commit 10. Logged rather than silent, so a message that
    ...
    private static async Task<bool> IsDuplicateAsync(ModerationDbContext db, string messageId, CancellationToken cancellationToken) =>
    ...
    {
        logger.LogError(args.Exception, "Moderation Service Bus processor error on {Subscription}", SubscriptionName);
