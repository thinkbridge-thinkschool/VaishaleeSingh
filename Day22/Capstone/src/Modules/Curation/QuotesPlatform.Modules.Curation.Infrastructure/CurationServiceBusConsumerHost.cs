...
/// <summary>
/// Competing-consumer worker for ServiceBusTopology.Subscriptions.CurationReviewDecisions.
///
...
///
/// This subscription carries TWO event types (CollectionApproved and
/// CollectionRejected) -- the dispatch is by EventType, not by subscription,
/// which is exactly why the handler lookup is keyed rather than singular.
/// </summary>
public sealed class CurationServiceBusConsumerHost(
    IServiceScopeFactory scopeFactory,
    ServiceBusClient serviceBusClient,
    ILogger<CurationServiceBusConsumerHost> logger) : BackgroundService
{
    private const string SubscriptionName = ServiceBusTopology.Subscriptions.CurationReviewDecisions;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        logger.LogInformation("Curation Service Bus processor started on {Subscription}", SubscriptionName);
        ...
        try
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // commit 11. Logged rather than silent, so a message that
    ...
    private static async Task<bool> IsDuplicateAsync(CurationDbContext db, string messageId, CancellationToken cancellationToken) =>
    ...
    {
        logger.LogError(args.Exception, "Curation Service Bus processor error on {Subscription}", SubscriptionName);
