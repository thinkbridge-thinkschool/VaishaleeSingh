...
| End-to-end run, all three flows | `Day30/verification/feature-complete-run.txt` |
| ADR-0002 | `Day28/docs/adr/0002-broadcast-handlers-commit-many-aggregates.md` |
...
So the honest statement was narrower than "verified" but much stronger than
this morning's: the business logic in every handler had run against a real
database; the messaging around it had not been re-proven.

**It has now been.** `Day30/verification/feature-complete-run.txt` is the
transcript of `happy-path.ps1` against SQL Server in Docker and the live dev
Service Bus namespace, with `catalog-quote-decisions` created by that same run.

The interesting part is the first three lines. On Day 29 they were three
synchronous POSTs to `mark-publishable`. They are now three quotes each making
three broker hops — submit, wait for the review to open, approve, wait for the
publishable flag — nine hops before the collection work starts. The script
waits on state rather than sleeping, so a wrong filter, a missing subscription
or a dead relay surfaces as a timeout rather than as a pass that breaks three
steps later.

What the transcript still does not cover, named rather than left to be noticed:
the **rejection path and the revision loop are not in that script**. They are
covered by the integration tests, against a real database but without the
broker. A scripted reject-and-resubmit against live infrastructure is the
